...
app.layout = dbc.Container([
    dbc.Row([
        dbc.Col([
            ...
            dbc.Button("Generate GTFS", id="generate-gtfs-btn", color="primary", className="mb-2"),
            dbc.Button("Run Model", id="run-model-btn", color="success", className="mb-4"),
            dbc.Card([
...
def generate_gtfs(geojson, n_clicks, current_geojson):
    ...
    if current_geojson:
        ...
        markers = [dl.Marker(position=pt) for pt in points]
        return f"✏️ {len(points)} points selected", markers, no_update, no_update
